...
public class ExpositionController {
    ...
    @ApiOperation(value = "Gets design data of expo")
    @ApiResponses({
            @ApiResponse(code = 200, message = "Design data found.")
    })
    @RolesAllowed("ROLE_EDITOR")
    @RequestMapping(method = GET, path = "/{id}/designData")
    public ResponseEntity<ExpositionDesignDataDto> design(@PathVariable String id) {
        ExpositionDesignDataDto design = expositionService.findDesign(id);
        return design == null ? ResponseEntity.noContent().build() : ResponseEntity.ok(design);
    }
