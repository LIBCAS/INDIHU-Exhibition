...
import ScreenDraggedCard from "./screen-dragged-card";
...
export default compose(
  ...
  withHandlers({
    ...
    isOverlapingPossibleTargetCard:
      ({ rectanglesOverlap }) =>
      (left, right, top, bottom, rowNum, colNum) => {
        const screen = document.getElementById(
          `expo-structure-screen-card-${rowNum}-${colNum}`
        );
  ...
  withHandlers({
    updateDragAndDropData:
      ({ dragAndDropData, setDragAndDropData, setOnClickEnabled }) =>
      (updateData) => {
          ...
          forEach(row, (_, colNum) => {
            ...
            if (screen) {
              ...
              draggedCard.style.left = `${
                screenPosition.left + container.scrollLeft
              }px`;
              draggedCard.style.top = `${
                screenPosition.top + container.scrollTop
              }px`;
              isInside = {
                rowNum,
                colNum,
    ...
    checkIfOverlapCard:
      ...
      (left, right, top, bottom) => {
        if (targetLocationCard && targetLocationCard.rowNum < screens.length) {
          const screen = isOverlapingTargetLocationCard(
            left,
            right,
            top,
            bottom
        ...
        if (!newTargetLocationCard) {
          forEach(screens, (firstScreen, rowNum) => {
            if (
              ...
              const screen = document.getElementById(
                `expo-structure-screen-card-${rowNum}-${
                  screens[rowNum].length - 1
                }`
              );
  ...
  withHandlers({
    onDraggingEnd:
      ({ resetDragAndDropData, mouseDownTimeout, setMouseDownTimeout }) =>
      () => {
