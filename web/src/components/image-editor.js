...
class ImageEditor extends Component {
  ...
  crop() {
    const canvas = this.cropperRef.current.getCroppedCanvas();
    if (canvas.height > 1 && canvas.width > 1) {
      this.cropperRef.current.replace(canvas.toDataURL());
    } else {
  ...
  cropMove() {
    ...
    this.cropperRef.current.setDragMode(newIsCrop ? "crop" : "move");
  }
  ...
  scaleHoriz() {
    ...
    this.cropperRef.current.scale(newScale[0], newScale[1]);
  }
...
export default compose(
  ...
  connect(null, { setDialog, setImageEditor }),
