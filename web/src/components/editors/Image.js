...
const Image = ({
  ...
  return (
    <div className="flex-row-nowrap">
      <div className={classNames("flex-col card-image-container", { img: !!image, "img-none": !image })}>
        <div>{title}</div>
        <Card className={classNames("card-image", { img: !!image, "img-none": !image })}>
          {image && <div
            ...
            onMouseUp={e => {
              ...
              if (infopoints || sequences || images) {
                const boundary = document.getElementById("screen-image-infopoints").getBoundingClientRect();
                updateScreenData(
                      ...
                      map(
                        ...
                        item =>
                          item.move
                            ? {
                              ...item,
                              left: (e.pageX - boundary.left + correlationX) / zoom,
                              top: (e.pageY - boundary.top + correlationY) / zoom,
                              move: false
                            }
                            : item
                      )
          ...
          >
            ...
            {imageWidth && imageHeight
              ...
              />}
            {/* INFOPOINTS */}
            {(infopoints || sequences || (images &&
              !isEmpty(find(images, img => img.active)) &&
              find(images, img => img.active).infopoints)) &&
              map(
                infopoints ? infopoints : sequences ? sequences : find(images, img => img.active).infopoints,
                (item, i) =>
                  item.move
                    ? <FontIcon
                      key={i}
                      ...
                      style={{
                        position: "absolute",
                        left:
                          mouseXPos - document.getElementById("screen-image-infopoints").getBoundingClientRect().left
                          + correlationX - infopointWidth / 2,
                        top:
                          mouseYPos - document.getElementById("screen-image-infopoints").getBoundingClientRect().top
                          + correlationY - infopointWidth / 2
                      }}
                    >
                      help_outline
                          </FontIcon>
                    : <div key={i}>
                      <FontIcon
                        ...
                        style={{
                          ...
                          animation: activePoint === i && "infopoint-pulse 2s infinite"
                        }}
                        onMouseDown={e => {
                          ReactTooltip.hide();
                          const boundary = document.getElementById(`screen-image-infopoint-${i}`).getBoundingClientRect();
                          mouseActualize({
                            mouseDown: true,
                            mouseXPos: e.pageX,
                            mouseYPos: e.pageY,
                            correlationX: boundary.left + infopointWidth / 2 - e.pageX,
                            correlationY: boundary.top + infopointWidth / 2 - e.pageY
                          });
                          updateScreenData(
                            infopoints || sequences
                              ? set(
                                ...
                                map(
                                  infopoints ? infopoints : sequences,
                                  (infopoint, idx) => i === idx ? { ...infopoint, move: true } : infopoint
                                )
                              )
                              : {
                                images: map(
          ...
          <div className="flex-row flex-space-between">
            <div>
              <FontIcon className="icon"
                onClick={() => zoom < defaultZoom + (5 * defaultZoom / 3) && zoomActualize(zoom + defaultZoom / 3)}
              >
                zoom_in
              </FontIcon>
              <FontIcon className="icon"
              ...
              <FontIcon className="icon" onClick={() => zoom > defaultZoom && zoomActualize(zoom - defaultZoom / 3)}>
                zoom_out
              </FontIcon>
            </div>
            <div>
              ...
              <FontIcon className="icon" onClick={() => !!onDelete && onDelete()}>
                delete
              </FontIcon>
            </div>
          </div>}
      </div>
      <HelpIcon {...{ label: helpIconText.EDITOR_IMAGE }} />
    </div>
  );
};
...
export default compose(
  connect(
    ({
      app: { mouseInfo: { mouseDown, mouseXPos, mouseYPos, correlationX, correlationY } }
    }) => ({
      mouseDown, mouseXPos, mouseYPos, correlationX, correlationY
    }),
  ...
  withHandlers({
    changeImage: ({ setDialog, setImage, setImageWidth, setImageHeight, setDragStyle, showLoader }) => () => {
    ...
    initVariables: ({ onLoad, setImageWidth, setImageHeight, setDefaultZoom, zoomActualize, showLoader }) => img => {
      const cardWidth = 450;
