...
export const loadScreen = url => async (dispatch, getState) => {
  ...
  if (!structure) setTimeout(() => dispatch(loadScreen(url)), 2000);
    ...
    const type = /.+\/screen\/.+\/.+\/(description|authors|documents|image|images|video|sequence|photogalery|parallax|text|answers|externalData)/.test(
      url
    )
      ? url.match(
...
export const saveScreen = (activeScreen, rowNum, colNum) => async (
  dispatch,
  getState
) => {
